    ...
    <>
      ...
      <aside className={`sidebar ${isOpen ? "open" : "closed"}`}>
            ...
            {projects.length === 0 ? (
              ...
              projects.map((project) => (
                ...
                >
                  <button
                    className={`nav-item ${
                      currentView === "project" && currentProjectId === project.id ? "active" : ""
                    }`}
                    onClick={() => handleViewChange("project", project.id)}
                  >
                    <span className="project-dot" style={{ backgroundColor: project.color }} />
                    <span className="project-name">{project.name}</span>
                    <span className="nav-badge">{getProjectTaskCount(project.id)}</span>
                  </button>
        ...
        <div className="sidebar-footer">
          <button className="shortcuts-button" onClick={onOpenShortcuts} aria-label="View keyboard shortcuts">
            <Keyboard size={16} />
            <span>Shortcuts</span>
          </button>
          ...
          <div className="footer-actions">
            <button className="footer-button" onClick={onSignOut} aria-label="Sign out">
              <LogOut size={18} />
            </button>
          </div>
        </div>
